...
export const WebSocketProviderForChat = ({ children }) => {
  ...
  const handleGameResponse = async (notificationId, accepted, gameId) => {
    ...
    try {
        if (accepted) {
            ...
            const response = await Axios.post(`/api/game/response/${gameId}/`, {
                action: 'accept'
            });
            ...
            await Axios.post(`/api/game/response/${gameId}/`, {
                action: 'decline'
            });
  ...
  const handleNotification = (data) => {
    ...
    // Add handler for game accepted notifications
    if (data.notification_type === 'game_accepted') {
